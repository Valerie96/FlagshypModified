...
OutsideNSetName = "nset=Outside"; 
% 
NewINP = "AttwoodCompression-1_100Fibers_doublelayer.inp";    %-Coarse

% OriginalINP = "INPfiles/RussellTensile-1-5.inp";
% PartName = "Gauge";
% OutsideNSetName = "nset=Outside"; 
% % 
% NewINP = "RussellTensile-1-5_12_doublelayer.inp";    %-Coarse
...
% OutsideNSetName = "nset=AllOutside"; 
% 
% NewINP = "KarthikeyanPlate_1000Fibers_doublelayer.inp";

OriginalINP = "INPfiles/FlagshypCube_8h_0t_PureShear.inp";
PartName = "Cube";
OutsideNSetName = "nset=CubeOut"; 

NewINP = "Test_doublelayer.inp";
...
%Create Matrix of truss nodes
    TrussX1 = zeros(ntrussx*nptsx,3);
    TrussY1 = zeros(ntrussy*nptsy,3);
    
%Define the distance between points on a fiber
    disty = (Ybound(2)-Ybound(1))/(nptsy-1);
    distx = (Xbound(2)-Xbound(1))/(nptsx-1);
%Create first layer of X and Y
%First trusses are not quite located on the edge of the bounds. First layer
%is not quite at the lowest z value
    xt = Xbound(1)+(DBT/4);
    yt = Ybound(1)+(DBT/4);    
    z1 = Zbound(1)+(DBT/4);
    for i=1:nptsx:size(TrussX1,1)
        ypt=Ybound(1);
        for j=0:nptsx-1
            TrussX1(i+j,:) = [xt, ypt, z1];
            ypt=ypt+disty;
        end
        xt=xt+DBT;
    end
    for i=1:nptsy:size(TrussY1,1)
        xpt=Xbound(1);
            for j=0:nptsy-1
                TrussY1(i+j,:) = [xpt, yt, z1];
                xpt=xpt+distx;
            end
        yt=yt+DBT;
    end

%Create second layer of X and Y to fit into gaps in the first layer
%Second layer is exactly the same coordinates as the first except shifted
%over by 1 truss radius
    TrussX2=TrussX1;
    TrussX2(:,1)=TrussX2(:,1)+DBT/2;

    TrussY2=TrussY1;
    TrussY2(:,2)=TrussY2(:,2)+DBT/2;
...
%Create matricies for each truss layer
    ...
    TrussXX = zeros(uint32(size(TrussX1,1))*ntrussz2,3);
    TrussYY = zeros(uint32(size(TrussY1,1))*ntrussz2,3);
...
%matricies and then add z coordinates
    ...
    for i=1:2:ntrussz2
        kx=1+(i-1)*size(TrussX1,1);
        ky=1+(i-1)*size(TrussY1,1);
        TrusXXCheck=TrussXX(kx:kx+size(TrussX1,1)-1,:);
        TrussXX(kx:kx+size(TrussX1,1)-1,:)=TrussX1;
        TrussYY(ky:ky+size(TrussY1,1)-1,:)=TrussY1;
        
        kx2=1+(i)*size(TrussX1,1);
        ky2=1+(i)*size(TrussY1,1);
        TrussXX(kx2:kx2+size(TrussX2,1)-1,:)=TrussX2;
        TrussYY(ky2:ky2+size(TrussY2,1)-1,:)=TrussY2;

        %Change z values
        %FIX THIS LATER YOU
        repeat_dist = 2*DBT+DBT*sqrt(3);
        TrussXX(kx:kx+size(TrussX1,1)-1,3)=ones(1,size(TrussX1,1))*(z1+double(i-1)/2*repeat_dist);
        TrussXX(kx2:kx2+size(TrussX2,1)-1,3)=TrussXX(kx:kx+size(TrussX1,1)-1,3)+(DBT/2)*sqrt(3);

        TrussYY(ky:ky+size(TrussY1,1)-1,3)=ones(1,size(TrussY1,1))*(z1+double(1+(i-1))*repeat_dist/2);
        TrussYY(ky2:ky2+size(TrussY2,1)-1,3)=TrussYY(ky:ky+size(TrussY1,1)-1,3)+(DBT/2)*sqrt(3);
    end
